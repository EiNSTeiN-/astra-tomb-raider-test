...
import { drawCausewayMap } from "./echo-causeway-map.js";
...
import { ORBIT_RECORD } from "./orbit-rules.js";
...
function init() {
  document.querySelector("#app").innerHTML = `
    ...
    <section id="game-screen" class="hidden" aria-label="Vesper adventure"><div id="game-canvas"></div><div class="game-vignette"></div><div id="damage-flash"></div>
      ...
      <div id="survey-scope" class="survey-scope hidden" aria-hidden="true"><div class="survey-reticle"></div><div class="survey-optic-label"><strong id="survey-optic-clue"></strong><span id="survey-optic-bearing"></span><span>Move / drag to sight · Use records · Jump leaves</span></div></div><div class="crosshair" aria-hidden="true"><span></span><span></span><i></i></div><div id="aim-status" class="aim-status" role="status"></div><div id="stealth-status" class="stealth-status hidden"><span class="stealth-bearing" aria-hidden="true">↑</span><span id="stealth-label" role="status"></span><div class="stealth-track" role="meter" aria-label="Guardian suspicion" aria-valuemin="0" aria-valuemax="100"><i></i></div></div><div class="sense-label hidden" id="sense-label">${icon("Eye")} EXPLORER’S INSTINCT</div><div id="waypoint" class="waypoint">◇<span></span></div>
...
function updateHUD(s) {
  const scope = s.desertSurvey?.focus;
  document.querySelector("#survey-scope").classList.toggle("hidden", !scope);
  document
    .querySelector("#game-screen")
    .classList.toggle("survey-looking", !!scope);
  if (scope) {
    document.querySelector("#survey-optic-clue").textContent = scope.label;
    document.querySelector("#survey-optic-bearing").textContent =
      `${scope.bearing.toFixed(1)}° bearing · ${scope.elevation.toFixed(1)}° elevation`;
  }
  ...
  if (s.desertSurvey)
    document.querySelector("#objective-detail").textContent =
      s.desertSurvey.text + " · M opens your chart";
  else if (s.arcadeLock)
    document.querySelector("#objective-detail").textContent =
      s.arcadeLock.text + " · M shows the lock";
  else if (s.sunBridge)
    document.querySelector("#objective-detail").textContent =
      s.sunBridge.text + " · M shows the spans";
  else if (s.shutterHouse)
    document.querySelector("#objective-detail").textContent =
      s.shutterHouse.text ||
      "Shutter house · Three catches per wheel · M shows the wind walks";
  else if (s.courier)
    ...
    document.querySelector("#objective-detail").textContent =
      "Optional climb · The Surveyor’s Cleft · M shows the wall elevation";
  else if (s.bellHoist)
    document.querySelector("#objective-detail").textContent =
  ...
  const controlLabel = (text) =>
    touchLabels
      ? text
          ...
          .replace(/\bE\b/g, "Use")
          .replace(/\bX\b/g, "Dive")
          .replace(/\bB\b/g, stealth.crouching ? "Stand" : "Crouch")
      : text;
...
function drawMap(canvas, full = false, s = game?.state()) {
  if (!game || !s) return;
  if (s.desertSurvey) return drawDesertSurveyMap(canvas, game, full);
  if (s.arcadeLock) return drawArcadeLockMap(canvas, game, full);
  if (s.sunBridge) return drawSunBridgeMap(canvas, game, full);
  if (s.shutterHouse) return drawShutterMap(canvas, game, full);
  if (s.causeway) return drawCausewayMap(canvas, game, full);
...
function showJournal() {
  ...
  const pressureSection = pressure?.visited
    ...
    : "";
  const cleft = store.data.levels.sands?.cleft;
  const cleftSection = cleft?.visited
    ? `<section class="tide-journal"><span class="eyebrow">THE SURVEYOR’S CLEFT · ${cleft.recovered ? "RECOVERED" : "IN PROGRESS"}</span><h3>${CLEFT_RECORD.title}</h3><p>${cleft.recovered ? CLEFT_RECORD.text : "A path east of the western survey station leads to the cleft. Follow the bronze handholds across the quarry face, rest on its three terraces, and recover the surveyors’ last bearing from the upper lookout."}</p>${cleft.recovered ? `<p class="small-copy">${CLEFT_RECORD.note}</p>` : ""}</section>`
    : "";
...
function showCleftGuide(recovered = false) {
  modal(
    recovered ? CLEFT_RECORD.title : "The Surveyor’s Cleft",
    recovered
      ? `<div class="note-paper"><p>${CLEFT_RECORD.text}</p><p class="small-copy">${CLEFT_RECORD.note}</p></div><p class="small-copy">Added to your field journal. Follow the balcony east to the return line.</p><button class="primary-button" data-close>Return to the lookout</button>`
      : `<p class="modal-description">“Leave the line for the next climber.”</p><div class="note-paper"><p>Start at the bronze handholds on the western foot of the wall. <kbd>E</kbd> (Use on touch) catches a grip. While hanging, directions follow the wall: <kbd>W</kbd> / Up climbs, Left and Right traverse, and Down descends. Move between the grips; the upper fork avoids the first broken span.</p><p>The camera faces the wall as you grip it. Look around nearby holds with your usual look controls; step onto a terrace to look all the way around.</p><p>At a broken span, hold a direction and press <kbd>Space</kbd> (Jump), then hold <kbd>E</kbd> / Use to catch the far side. Open <kbd>M</kbd> for a front view of the wall. Dashed links mark the longer jumps.</p><p>Beside a terrace, release the direction and press Jump to step onto it. Resting restores stamina. Use again while hanging releases you to the belay line; a missed catch or exhaustion also returns you to your last rest terrace. Reloading during a climb restores that terrace.</p><p>The last bearing rests on the summit writing desk. A return line at the eastern end of the upper balcony lowers you to the sand.</p></div><button class="primary-button" data-close>Explore the cleft</button>`,
    "guide",
  );
}
...
function showPressureGuide(record = false) {
  modal(
    record ? PRESSURE_RECORD.title : "The Cinder Relay",
    record
...
function showOrbitGuide(record = false) {
  modal(
    record ? ORBIT_RECORD.title : "The Cartographer’s Orrery",
    record
      ? `<div class="note-paper"><p>${ORBIT_RECORD.text}</p></div><p class="small-copy">The western bridge is released. Close the chart and wait for its leaves to unfold before crossing.</p><button class="primary-button" data-close>Return to the instrument</button>`
