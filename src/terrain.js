...
import { ARCADE_SITE, arcadeFoundationWeight } from "./arcade-lock-rules.js";
import {
  SURVEY_FOUNDATIONS,
  surveyFoundationAt,
...
export function createTerrainProfile(map, level) {
  ...
  const terraces = [
    ...
    ...(map.cleft ? [map.cleft] : []),
    ...(map.pressureRelay ? [map.pressureRelay] : []),
  ].map((r) => ({
    x: r.x * 7,
    z: r.z * 7,
    radius: r.r * 7,
  ...
  for (let iz = 0; iz < width; iz++)
    for (let ix = 0; ix < width; ix++) {
      ...
      if (map.arcadeLock) {
        const weight = arcadeFoundationWeight(x, z);
        height =
          height * (1 - weight) + raw(ARCADE_SITE.x, ARCADE_SITE.z) * weight;
      }
      if (map.shutterHouse) {
      ...
      if (map.desertSurvey) {
        const site = surveyFoundationAt(x, z);
        if (site)
          height =
            height * (1 - site.weight) + raw(site.x, site.z) * site.weight;
      }
  ...
  const profile = {
    ...
    arcadeY: map.arcadeLock ? raw(ARCADE_SITE.x, ARCADE_SITE.z) : null,
    surveyBases: map.desertSurvey
      ? SURVEY_FOUNDATIONS.map((p) => raw(p.x, p.z))
      : null,
