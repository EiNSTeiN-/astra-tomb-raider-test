...
| VA-19 | Jungle tree scans expose some lower root margins above sloped terrain. | The [jungle root repair](jungle-roots.md) centres both models on their lower trunk, fits a common footprint across all distance tiers to the rendered ground, and moves unsuitable placements to nearby soil. Verification covers 68,397 rendered root vertices, 24 bank views, an approach/return route, and native movement/reload checks. | Fixed |
...
| VA-24 | A jungle trunk has a narrow band of visibly stretched bark texture. | The [root-placement review](jungle-roots.md#remaining-art-review) records the strip beside the trunk of `island_tree_01` in the outer-bank close view. Inspect the delivered trunk UVs and texture mapping. | Open; jungle bark mapping |
