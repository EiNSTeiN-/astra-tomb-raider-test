...
export function reflectorPosition(g) {
  const r = g.easternReflector,
    p = g.player.position;
  return {
    ...
    raised: r.saved.raised,
    moving: !!r.motion,
    angle: r.panel.rotation.x,
    hint: g.state().traversalHint,
    objective: g.state().objective,
  };
}

export function reflectorTick(g, count = 1) {
...
export async function reflectorMove(g, points) {
  const r = g.easternReflector;
  g.yaw = 0;
  g.keys.clear();
  for (const [x, z] of points) {
    ...
    if (!reached)
      throw new Error(
        `Blocked to ${x},${z}: ${JSON.stringify(reflectorPosition(g))}`,
      );
  }
  g.touchMove = { x: 0, z: 0 };
  reflectorTick(g, 2);
  return reflectorPosition(g);
}

// Set direction before native Space / Jump, then advance the returned frame
// count. Release direction afterward and allow the character to settle.
export function reflectorJump(g, direction) {
  g.yaw = 0;
  g.keys.clear();
  g.touchMove = {
    x: direction === "west" ? -2 / 3 : 0,
    z: direction === "gallery" ? -2 / 3 : 0,
  };
  return direction === "west" ? 57 : 50;
}
